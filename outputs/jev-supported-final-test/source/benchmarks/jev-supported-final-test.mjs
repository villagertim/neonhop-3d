...
if(process.argv[2]==='prepare'){
 ...
 const results=traces.map(t=>{const c=cases.find(c=>c.id===t.stateId);const a=t.response?.answers?.action?.choice;return {...t,grade:ACTIONS.includes(a)?replay(c.snapshot,a):null,local:c.local,bestProgress:Math.max(...Object.values(c.candidates).filter(x=>x.safe).map(x=>x.progressScore))};});
