...
document.addEventListener('DOMContentLoaded', () => {
    ...
    jevAgent.onTelemetry = (data) => {
        ...
        const evacPct = Math.round(data.evacuateLog * 100);
        if (hudEvacBar) hudEvacBar.style.width = `${evacPct}%`;
        if (hudEvacVal) hudEvacVal.textContent = `${evacPct}%`;

        const fwdPct = Math.round(data.forwardClear * 100);
        if (hudForwardBar) hudForwardBar.style.width = `${fwdPct}%`;
        if (hudForwardVal) hudForwardVal.textContent = `${fwdPct}%`;
    ...
    engine.onGameOver = () => {
        if (engine.aiMode) {
            ...
            aiRestartTimer = setTimeout(() => {
                if (engine.aiMode && engine.state === 'GAME_OVER') {
                    engine.resetGame();
                    sound.setLevel(1);
                    sound.startMusic();
    ...
    function renderCallback(game, alpha) {
        ...
        switch (game.state) {
            ...
            case 'GAME_OVER':
                ...
                if (leaderboard.isQualified(game.score)) {
