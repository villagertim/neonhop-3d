...
export class PhysicsOracle {
    constructor() {
        this.hopDuration = 0.134;  // 8 physics frames (8 / 60s)
        this.minDwellTime = 0.120; // Time frog stays on tile before next move
    ...
    isPositionSafeOverTime(targetX, targetZ, tLanding, tDwell, engine) {
        ...
        if (targetZ >= 1 && targetZ <= 5) {
            ...
            const minSafeRunwayTime = 0.30; // 300ms runway
    ...
    evaluateActionSafety(engine, action, lastAction = null) {
        ...
        if (closestPortal && currZ >= 1 && currZ <= 5) {
            ...
            if (currZ >= 2) {
                ...
                if (action === 'UP') {
                    // Target lane Z = currZ - 1:
                    // If target lane moves LEFT (Lane 4 from 5, Lane 2 from 3), penalize entering at x < -1.0 / -2.0
                    // If target lane moves RIGHT (Lane 3 from 4, Lane 1 from 2), penalize entering at x > 2.2
                    if (currZ === 3 && currX < -3.2) {
                        score -= 2500; // Entering Lane 2 at extreme left has insufficient runway
                    } else if (currZ === 5 && currX < -2.0) {
                        score -= 2000;
                    } else if (currZ === 4 && currX > 2.2) {
                        score -= 2000;
                    } else {
                        score += 300; // Prioritize forward crossing!
                    }
                }
    ...
    verifyLookaheadEscape(engine, primaryAction, lastAction = null) {
        ...
        // Check if from (evalPrimary.targetX, evalPrimary.targetZ), at least one move exists
        const simulatedEngine = {
            player: {
                currPosition: { x: evalPrimary.targetX, y: 0, z: evalPrimary.targetZ },
                isHopping: false
            },
            obstacles: engine.obstacles.map(o => ({
                ...o,
                x: this.predictObstacleX(o, this.hopDuration)
            })),
            ...
            findOverlappingCar: (x, z) => {
                return engine.obstacles.find(obs => {
                    ...
                    const simulatedCarX = this.predictObstacleX(obs, this.hopDuration);
            ...
            findOverlappingLog: (x, z) => {
                return engine.obstacles.find(obs => {
                    if (obs.laneZ !== z || obs.type !== 'log') return false;
                    const halfLen = obs.length / 2;
                    const simulatedLogX = this.predictObstacleX(obs, this.hopDuration);
